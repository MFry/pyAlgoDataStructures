...
def get_checksum(spreadsheet):
    ...
    for i in range(len(spreadsheet)):
        row_max = max(spreadsheet[i])
        row_min = min(spreadsheet[i])
...
class MyTestCases(unittest.TestCase):
    def test_get_checksum(self):
        test_case = [[5, 1, 9, 5], [7, 5, 3], [2, 4, 6, 8]]
        self.assertEquals(get_checksum(test_case), 18)


if __name__ == '__main__':
    data = "493	458	321	120	49	432	433	92	54	452	41	461	388	409	263	58\
    ...
    data_rows = 16
    data = data.split()
    formatted_data = [[int(data[i*16+j]) for j in range(data_columns)] for i in range(data_rows)]
    print(formatted_data)
    print(get_checksum(formatted_data))
